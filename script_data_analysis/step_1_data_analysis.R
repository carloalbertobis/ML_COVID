...
saveRDS(plot_intro_ein_lite, file = paste0(output_dir,"plot_intro_ein_lite.rds"))
plot_intro_ein<- plot_intro(data_covid_ein)
saveRDS(plot_intro_ein, file = paste0(output_dir,"plot_intro_ein.rds"))
plot_ein_no_IMP<- plot_intro(data_covid_ein_no_IMP)
saveRDS(plot_ein_no_IMP, file = paste0(output_dir,"plot_ein_no_IMP.rds"))
...
saveRDS(table_ein_lite, file = paste0(output_dir,"table_ein_lite.rds"))
table_ein <- summary(tableby(cov_result ~ ., control = my_controls, test = FALSE, data = data_covid_ein[,c(-1,-2)]), text = TRUE)
saveRDS(table_ein, file = paste0(output_dir,"table_ein.rds"))
...
saveRDS(table_covid_vs_age_lite, file = paste0(output_dir,"table_covid_vs_age_lite.rds"))
saveRDS(table_covid_vs_age, file = paste0(output_dir,"table_covid_vs_age.rds"))
...
saveRDS(table_ser_vs_age_lite, file = paste0(output_dir,"table_sev_vs_age_lite.rds"))
saveRDS(table_sev_vs_age, file = paste0(output_dir,"table_sev_vs_age.rds"))




rm(table_sev_vs_age, table_ser_vs_age_lite, table_covid_vs_age, table_covid_vs_age_lite, table_ein, table_ein_lite)
rm(data_covid_ein, data_covid_ein_lite, data_covid_ein_no_IMP)
rm(plot_intro_ein_lite, plot_intro_ein, my_controls, plot_ein_no_IMP)
